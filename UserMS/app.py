...
def register_user(user_register_body):
    ...
    if found_user:
        ...
        if user_register_body['password'] == user_register_body['confirm_password']:
            ...
            address = user_register_body['address']
            phone = user_register_body['phone']
            ...
            new_user = User(username=username, password=password, email=email, age=age, name=name, surname=surname,
                            address=address, phone=phone, is_admin=is_admin)
...
def delete_user(user_id):
    ...
    db.session.commit()
    return {'Response': 'User with id: {} is deleted successfully!'.format(user_id)}, 200
